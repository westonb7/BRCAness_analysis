...
writefile = open("mutec_2_aggregated.tsv", "w")

spazer = 0
counter = 0

writefile.write("Sample ID\tChromosome\tPosition\tRef\tAlt\tGene Name\tSS ID\tNormal_RD\tNormal_AD\tTumor_RD\tTumor_AD\n")

#readfile = open("filenames_vcf.txt", "r")
#for line in readfile:
for som_file in glob.glob("/Data/TCGA/SomaticMutations/Mutect2_VCF_BRCA_hg19/*reheader.vcf"):
#	line = line.strip()
			...
			if FILTER == "PASS":
				geneFile = open("gencode.v19.annotation.BROCA2.bed", "r")
					...
					if (chrom == chroms) and (position < gene_end) and (position > gene_start) and (ss_num == "2" or ss_num == "3"):
						writefile.write(tcga_ID.group(1) + "\t" + chrom + "\t" + position + "\t" + ref + "\t" + alt + "\t" + gene_name + "\t" + ss_id + "\t" + norm[3] + "\t" + norm[4] + "\t" + tumr[3] + "\t" + tumr[4] + "\n") 
						#print gene_name
